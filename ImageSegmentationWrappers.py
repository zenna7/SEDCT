...
class TorchImageSegmentation(BaseImageSegmentation):
    ...
    def remove_segment(self, segment: Union[int, list[int]], 
        ...
        if self._n_segments is not None:
            if not all(x <= self._n_segments for x in segment):
        ...
        if fill_method is not None:
            ...
            warnings.warn("No method to fill the removed segment(s) was specified. 'fill' method has been used as default.", category=UserWarning)
...
class SKLearnImageSegmentation(BaseImageSegmentation):
    def __init__(self, image: Union[Tensor, ndarray, Image, list],
                 segmentation_method: Optional[str] = None,
                 fill_method: Optional[str] = None,
                 **kwargs) -> None :
