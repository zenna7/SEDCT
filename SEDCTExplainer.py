...
class SEDCTExplainer(): 
    ...
    def _check_compatibility(self) -> None:
        ...
        if self.classifier is not None and self.image_segmentation is not None:
            if isinstance(self.classifier, TorchModelWrapper) and isinstance(self.image_segmentation, SKLearnImageSegmentation):
                raise ValueError(f"A classifier of the class TorchModelWrapper does not support images in numpy.ndaray format coming from a "
                                f"object of the class SKLearnImageSegmentation.")
    ...
    def explain_instance(
            ...
            max_iter: Optional[Union[int, float]] = -1, 
            show: Optional[bool] = False,
            compatibility_check: Optional[bool] = True
            ) -> None : 
        import numpy as np
        ...
        if not isinstance(compatibility_check, bool):
            raise TypeError(f'compatibility_check parameter must be of type bool. Provided type ({type(show)}) is incorrect.')
